#!/usr/bin/env python3
"""
Integrating the exact transcription code provided with Azure Storage and SQL

This script will:
1. Get a blob from Azure Storage and generate a SAS URL
2. Use the provided transcription code to transcribe the audio
3. Store the results in Azure SQL database
"""
...
# ============================
# MODIFIED TO WORK WITH SAS URL
# ============================

def transcribe_from_url(url, api_key, model="nova-2", diarize=True):
    """
    Transcribe audio from a URL using Deepgram API
    
    Args:
        url: URL to the audio file (SAS URL)
        api_key: Deepgram API key
        model: Deepgram model to use
        diarize: Whether to enable speaker diarization
    ...
    print(f"Transcribing from URL using Deepgram API...")
    print(f"Model: {model}, Speaker Diarization: {'Enabled' if diarize else 'Disabled'}")
    
    try:
        # Send request to Deepgram
        print("Sending request to Deepgram, please wait...")
...
def main():
    ...
    try:
        ...
        # Step 3: Transcribe using the provided code (modified for URL)
        logger.info("Transcribing with Deepgram...")
        transcription_result = transcribe_from_url(
            url=blob_sas_url,
...
if __name__ == "__main__":
    logger.info("Starting direct transcribe test...")
    success = main()
